var appmgrMiniProgressApp = angular.module('appmgr-mini-progress', ['common-service']);

appmgrMiniProgressApp.controller('appmgrMiniProgressCtrl', ['$scope', 'commonService',
    function ($scope, commonService) {
        ...
        $scope.firstProgressSignal = true;
        $scope.showTimeRemaining = false;
        $scope.pauseClicked = false;
        $scope.activeStatus = " ";
        $scope.installedAssets = 0;
        $scope.totalAssets = 0;
        ...
        $scope.pause = function (isPauseClicked) {
            var cmd = {
                ...
                Action: 'pause',
                IDList: $scope.IDList,
                Cancel: !isPauseClicked
            };
            $scope.invokeCommand(cmd, isPauseClicked);
        };

        $scope.afterPause = function (isPauseClicked) {
            $scope.pauseClicked = isPauseClicked;
            $scope.showCancelConfirm = !isPauseClicked;
            if (isPauseClicked) {
                $scope.showPause = false;
                $scope.showTimeRemaining = false;
                $scope.activeStatus = $scope.cs.allPaused;
            }
        };
        ...
        $scope.invokeCommand = function (cmd, isPauseClicked) {
        ...
        $scope.advise = function (advice) {
            if (advice.Source == 'AssetMgr' && advice.Signal == 'assetChanged') {
                if (advice.State == 14) {
                    $scope.installedAssets++;
                    var dRatio = $scope.installedAssets / $scope.totalAssets,
                        installProgress = {
                            "elementId": "mini",
                            "elementWidth": 198,
                            "downloadRatio": isNaN(dRatio) ? 0 : dRatio,
                            "downloadFlag": false
                        };
                    $scope.cs.progressBar(installProgress, false);
                }
            } else if (advice.Source == 'InstallMgr' && advice.Signal == 'overallProgressChanged') {
                var active = parseInt(advice.ActiveDownloads, 10),
                    paused = parseInt(advice.PausedDownloads, 10),
                    pendingDownloads = parseInt(advice.PendingDownloads, 10),
                    downloading = parseInt(advice.Downloadings, 10),
                    pendingInstalls = parseInt(advice.PendingInstalls, 10),
                    installings = parseInt(advice.Installings, 10),
                    downloadSpeed = advice.DownloadSpeed,
                    downloadedBytes = advice.DownloadedBytes,
                    totalDownloadBytes = advice.TotalDownloadBytes,
                    remainingTime = advice.RemainingTime,
                    dRatio = (active == pendingInstalls + installings) ? 1 : (downloadedBytes / totalDownloadBytes);

                $scope.IDList = advice.DownloadIDs;
                $scope.showTimeRemaining = (downloadSpeed > 0 ? true : false);
                if ((active == paused) || ($scope.pauseClicked && (installings == 0))) {
                    $scope.activeStatus = $scope.cs.allPaused;
                } else if ($scope.pauseClicked && installings == 1) {
                    if (active > 1) {
                        $scope.activeStatus = installings + " " + $scope.cs.activeText + " / " + (active - 1) + " " + $scope.cs.pausedText;
                    } else {
                        $scope.activeStatus = installings + " " + $scope.cs.activeText;
                    }
                } else if (paused == 0) {
                    $scope.activeStatus = active + " " + $scope.cs.activeText;
                } else {
                    $scope.activeStatus = (active - paused) + " " + $scope.cs.activeText + " / " + paused + " " + $scope.cs.pausedText;
                }
                if (active == installings) {
                    $scope.disableCtrls = true;
                }
                if ($scope.firstProgressSignal) {
                    $scope.showPause = (active == installings + paused ? false : true);
                    $scope.totalAssets = advice.DownloadIDs.length;
                    $scope.firstProgressSignal = false;
                }
                var miniProgress = {
                    "elementId": "mini",
                    "elementWidth": 200,
                ...
                $scope.cs.progressBar(miniProgress, true);
            }
        };
        ...
        /**********************************************************************************************
         ************************************* Mini progress events ***********************************
         **********************************************************************************************/
        angular.element(document).ready(function () {
            arb.localizeHtml();
            loadLocaleStyles("mini-progress");
            try {
...
// SIG // AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFID5IA/zupOV
// SIG // 3olYJretbXhWheUvoIIUMDCCA+4wggNXoAMCAQICEH6T
...
// SIG // 9w0BCQQxFgQU7a/U7fcRwUbQ3HWcVGjTXqbDK9UwVAYK
...
// SIG // AQBGAd6Oe2gmz5h5L8dUDXUi+abofzfvrlKsHeH3Kj4O
// SIG // xSYD+aUaCwQh0FU8KOLqqfgV3TIOj/et0w67P8Q24/gA
// SIG // HHcPY4n7CvfSIJHpiCL5bHUTdF22N1VMs4vXgd6fvWpR
// SIG // Oaj+rL2mu21V+donJNmE9kyRgeHSDd6eFJ+g/QXuuwX7
// SIG // FlrwpRrIwG/jA6ew95Ge0YJrSr3BhSI3aLVnmikIIk4Q
// SIG // HPMWLKARTW+sjySS75xM4fDphckBikVruNFcfOj46lCa
// SIG // LJi1kKIz6SK66GS60yVitOEO4wNve3zP4FlLhPqfJ2ry
// SIG // Ve1tsvvlHX2qZxKxuPfYN9yS9EzIZRXBaGIVoYICCzCC
...
// SIG // MTIwNTAyMjc1MFowIwYJKoZIhvcNAQkEMRYEFDnpAxoc
// SIG // e1ckQqy3rt1h8wKjo9BUMA0GCSqGSIb3DQEBAQUABIIB
// SIG // AGmGQUIZmfowvC0aZ2ZWbUxfbQ6UVlXw4r1oqIcatfzL
// SIG // bkcrsZmAjf5I2/ckRmEiectGJKpUl+n+S/5V3Es7pXqD
// SIG // g9eLeh9OAao851HsyX/seLTBzxH9NAxwzLuNh/Skes1T
// SIG // rquSZnGu/RtIqsYVhHuFlAilEFLcJEHvAxgkVsp46xDv
// SIG // JjzXHtXZCl0dGqQxQxxoQXN2RFhSgdDRTVDogYcca4JL
// SIG // QCGGgD7jaVQkpmAF0ACd2YVcwoGRv3Xc14/oUGCfQMuV
// SIG // 8EVZIZsRRa9OFbIsfFDHTNQBvumf23VPr/0cTx1C6S5T
// SIG // 09sJELr5vJY4nCjwCT8uVDSLn+Ww0QKc8Lg=
