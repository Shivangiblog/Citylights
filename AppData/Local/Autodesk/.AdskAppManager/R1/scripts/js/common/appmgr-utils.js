var r$ = null,
    langCode = CmdTarget.getLocale();
/* Method to load arb file based on OS language settings */
function loadCulture() {
    getScript(langCode + '/appman_locale.arb');
    if (langCode != "en_US") {
        getScript('js/lib/cultures/globalize.culture.' + langCode + '.js');
    }
}
...
/* Method to set arb resource selector  */
function arbResourceSelector() {
    arb.setResourceSelector(langCode);
    r$ = arb.getResource("arb_editor");
    Globalize.culture(langCode.replace('_', '-'));
}

/* Load the language specific css rules */
function loadLocaleStyles(pagename) {
    var rules = asianFonts() + pageStyles(pagename);
    $("<style type='text/css'></style>").appendTo("head").append(rules);
}

/* Asian font */
function asianFonts() {
    if (langCode == "zh_CN") {
        return "html,body{font-family: 'Microsoft YaHei',Tahoma,Verdana,SimSun}";
    } else if (langCode == "zh_TW") {
        return "html,body{font-family: 'Microsoft JhengHei',PMingLiU}";
    } else if (langCode == "ja_JP") {
        return "html,body{font-family: Meiryo,'MS UI Gothic'}";
    } else if (langCode == "ko_KR") {
        return "html,body{font-family: 'Malgun Gothic',Gulim}";
    } else {
    	return "";
    }
}
...
function pageStyles(pagename) {
    var rules = "";
    switch (pagename) {
    case "main":
        if (langCode == "cs_CZ" || langCode == "hu_HU" || langCode == "ru_RU" || langCode == "ja_JP") {
            rules += ".asset .btn-progress-td {width: 225px}";
            rules += ".asset .progress-wrapper {width: 225px}";
            rules += ".progress-detail-info {padding-left: 55px;width: 170px}";
            rules += ".progress-wrapper .paused-text {width: 80px}";
        }
        if (langCode == "es_ES") {
            rules += ".asset .detail-wrapper td.col1{min-width: 170px;}";
        } else if (langCode == "cs_CZ" || langCode == "de_DE" || langCode == "it_IT") {
            rules += ".asset .detail-wrapper td.col1{min-width: 155px;}";
        }
        break;
    case "privacy":
        if (langCode == "hu_HU") {
            rules += ".content {padding: 50px 140px 0 135px;}";
        }
        break;
    case "self-update":
    case "updates-available":
    	if (langCode == "fr_FR" || langCode == "it_IT") {
            rules += ".notification .title {width: 192px}";
            rules += ".notification .time {width: 128px}";
        }
        break;
    }
    return rules;
...
// SIG // AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFAUTKB5Bxyhh
// SIG // h9XHHKY3NpJG0ohcoIIUMDCCA+4wggNXoAMCAQICEH6T
...
// SIG // 9w0BCQQxFgQUvjmVtJOMZm4vDJ9Rim15WLVLS9AwVAYK
...
// SIG // AQAfmqTd5cbu61OksbJ6eYGbsDuGA5hqTD5Eqh3Z2Fep
// SIG // 2FJhQk+T4SVkDUCTEKXJKrGX1t27RGPOpaGqTjGiFR3q
// SIG // YSRwcz3fzZU9/koIF/q18T+v9rwsAoKnzsMwYqE11Hq5
// SIG // 4e2EQDcOZtuPs4R1mHI/YdI4ea7gbdz5+IPqCbov/J10
// SIG // UumJOrpuoEiXJI6Z/w7KtxVdkCuHAtWlfJm/M33Ag8Kv
// SIG // vAipYTx5+xhc78nEvXnCvmqC04A3LvmcQR/o2AuGG3V8
// SIG // j64MZLqIj2fedXjBGGdTCOyHs82YfF0pQzGf4YmDrVDa
// SIG // kCp30wgZLneFfoIdPLYsMDX8mPy01No9uizvoYICCzCC
...
// SIG // MTIwNTAyMjc1MFowIwYJKoZIhvcNAQkEMRYEFAWahcj/
// SIG // 20gdmiV8cDfG8d0P0PQ2MA0GCSqGSIb3DQEBAQUABIIB
// SIG // AHFHbrjqHILwMNKrQOeR3RNW4dQ6ur1TLCFpi8xB0sIe
// SIG // Cn+h1nMs/y6qbjZZjOgFaFDOrja5v2MMdjbJBIYmsY0G
// SIG // L72xOdr2liuiMYoB4hBc+RQoeK4wNs6t9Wdes5cQ4bFM
// SIG // 24vMxpR9lmlYhsNRQnjWawPX7tXO8AJY5HxBQAQRqFhr
// SIG // wVQf/RHGSJ7xuURg+bfxyAIoehRs2GtaYKAlgORYIZ4W
// SIG // Dsg3pfi6Mpqn5IIIuVVc5jG2Q7Xm7OzhgHiprzgTTeDK
// SIG // 5xCKP9xjIIOqyFmDzLTLhkoa54JQ7KI9qz+xbBMVm2PL
// SIG // sXLuqmc7RiXHjVPXL1rbabof1KvFu53FZVU=
