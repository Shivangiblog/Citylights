				...
				if (remainingText) {
					const newPosition =
						this.lastCompletionTranslation > 0
							? position.translate(0, this.lastCompletionTranslation)
							: position;
		...
		return new Promise((resolve) => {
			timerId = setTimeout(async () => {
				...
				try {
					...
					const { text, indexSuffix, autocompleteId, model, provider, error } =
						await response.json();
					...
					if (text) {
						...
						if (indexSuffix >= 0) {
							...
							replaceRange = new Range(
								position,
								position.translate(0, lastCompletionTranslation),
							);
						}
						const completion = [
